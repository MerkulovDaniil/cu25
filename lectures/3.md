...
* Мы можем использовать множество алгоритмов для решения этой задачи. Однако, учитывая современный размер задачи, где $d$ может достигать десятков миллиардов, это очень сложно решить без информации о градиентах, используя алгоритмы нулевого порядка. 
...
Рассмотрим двухточечную оценку градиента^[предлагаю [хорошую](https://scholar.harvard.edu/files/yujietang/files/slides_2019_zero-order_opt_tutorial.pdf) презентацию о методах без градиента] $G$:
...
Для 2-точечных оценок, мы не можем сделать зависимость лучше, чем на $\sqrt{n}$ !
...
**Ответ** $2dT$, что очень долго для больших задач. Кроме того, этот точный метод нестабилен, что означает, что нам придется выбирать между точностью и стабильностью.
...
:::{.callout-question}
Обратите внимание, что для того же количества вычислений, что и в прямом режиме, мы получаемполный вектор градиента $\nabla_w L$. Это обходится нам бесплатно? Какова стоимость ускорения?
...
**Ответ** В общем случае невозможно ответить без некоторого знания о конкретной структуре графа $G$. Следует отметить, что существует множество продвинутых подходов для смешивания прямого и обратного режимов AD, основанных на конкретной структуре $G$.
...
* $v_0 = x$ на вход обычно подаётся пакет (batch) данных $x$
* Для $k = 1, \ldots, t-1, t$: 
    
    * $v_k = \sigma(v_{k-1}w_k)$. Обратите внимание, что на практике, данные имеют размерность $x  \in \mathbb{R}^{b \times d}$, где $b$ - размер батча (для одного данного $b=1$). В то время как матрица весов $w_k$ $k$ слоя имеет размер $n_{k-1} \times n_k$, где $n_k$ - размер внутреннего представления данных. 
