...
## Квадратичная сходимость метода Ньютона
...
Общий алгоритм квазиньютоновских методов основан на выборе матрицы $B_k$ так, чтобы она в некотором смысле стремилась к истинному значению гессиана $\nabla^2 f(x_k)$ при $k \to \infty$. Конечно, важно при этом сделать так, чтобы вычисления были не слишком дорогими.

## Схема построения квазиньютоновских методов
...
Разные квазиньютоновские методы реализуют шаг 3 по-разному. Мы скоро увидим, что часто можно вычислить $(B_{k+1})^{-1}$ из $(B_{k})^{-1}$.
\pause
...
## Симметричное одноранговое обновление для обратной матрицы
...
Помимо вывода $B_{k+1}$ из $B_k$, давайте получим выражения для вывода обратной матрицы, т.е. $C_{k+1} = (B_{k+1})^{-1}$ из $C_k = B_k^{-1}$.
\pause
...
## Обновление Бройдена — Флетчера — Гольдфарба — Шанно
...
## Обновление Бройдена — Флетчера — Гольдфарба — Шанно с инверсией
...
## Бонус: Идея методов адаптивной метрики
