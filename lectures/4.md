---
title: "Выпуклость: выпуклые множества, выпуклые функции. Условие Поляка - Лоясевича. Сильная выпуклость"
...
:::
Выпуклый конус: множество, содержащее все конические комбинации точек в множестве
:::
::: {.column width="60%"}
...
Это единичная окружность, с центромв начале координат. И:

$$
S_2 := \{x \in \mathbb{R}^2 : -4 \leq x_1 \leq -1, -3 \leq x_2 \leq -1\}
$$

Это прямоугольник. Сумма множеств $S_1$ и $S_2$ образует увеличенный прямоугольник $S_2$ с закругленными углами. Полученное множество будет выпуклым.
:::
::: {.column width="40%"}
...
![Пересечение полуплоскостей](conv_inter.pdf){width=60%}
...
Примеры аффинных функций: расширение, проекция, транспонирование, множество решений линейного матричного неравенства $\left\{ x \mid x_1 A_1 + \ldots + x_m A_m \preceq B\right\}$. Здесь $A_i, B \in \mathbf{S}^p$ симметричные матрицы $p \times p$. 

Также обратите внимание, что прообраз выпуклого множества при аффинном отображении также является выпуклым.
...
Пусть $x \in \mathbb{R}$ - случайная величина с заданным распределением вероятностей $\mathbb{P}(x = a_i) = p_i$, где $i = 1, \ldots, n$, и $a_1 < \ldots < a_n$. Тогда вектор вероятностей $p \in \mathbb{R}^n$ принадлежит вероятностному симплексу, т.е. 

$$
P = \{ p \mid \mathbf{1}^Tp = 1, p \succeq 0 \} = \{ p \mid p_1 + \ldots + p_n = 1, p_i \ge 0 \}.
$$

Определить, являются ли следующие множества $p$ выпуклыми:
...
Если вышеуказанное неравенство выполняется строгим неравенством $x_1 \neq x_2$ и $0 < \lambda < 1$, то функция называется строго выпуклой на $S$.
:::
::: {.column width="60%"}
...
называется **эпиграфом** функции $f(x)$. 

:::{.callout-theorem}

## Выпуклость эпиграфа = выпуклость функции

Для того чтобы функция $f(x)$, определенная на выпуклом множестве $X$, была выпуклой на $X$, необходимо и достаточно, чтобы эпиграф функции $f$ был выпуклым множеством.
:::
:::
::: {.column width="50%"}
![Эпиграф функции](epigraph.pdf)
:::
::::
...
1. **Необходимость**: Предположим, что $f(x)$ выпукла на $X$. Возьмем любые две произвольные точки $[x_1, \mu_1] \in \text{epi}f$ и $[x_2, \mu_2] \in \text{epi}f$. Также возьмем $0 \leq \lambda \leq 1$ и обозначим $x_{\lambda} = \lambda x_1 + (1 - \lambda) x_2, \mu_{\lambda} = \lambda \mu_1 + (1 - \lambda) \mu_2$. Тогда,
   ...
   Неравенство выше означает, что $\begin{bmatrix}x_{\lambda}\\ \mu_{\lambda}\end{bmatrix} \in \text{epi}f$. Таким образом, эпиграф функции $f$ является выпуклым множеством.


## Выпуклость эпиграфа = выпуклость функции

2. **Достаточность**: Предположим, что эпиграф функции $f$, $\text{epi}f$, является выпуклым множеством. Тогда, из принадлежности точек $[x_1, \mu_1]$ и $[x_2, \mu_2]$ эпиграфу функции $f$, следует, что
...
которое представляет собой эпиграф функции $x \mapsto \Vert x \Vert$. Это множество называется конусом норм. Согласно утверждению выше, множество $K$ является выпуклым. [\faPython Код для рисунков](https://colab.research.google.com/github/MerkulovDaniil/optim/blob/master/assets/Notebooks/Norm_cones.ipynb)

![Конусы норм для разных $p$ - норм](norm_cones.pdf)

## Множество субуровня

:::: {.columns}
::: {.column width="50%"}
![Множество субуровня функции с уровнем $\beta$](sublevel_set.pdf)
:::
::: {.column width="50%"}
...
называется **множеством субуровня** или множеством Лебега функции $f(x)$. 

. . .

Обратите внимание, что если функция $f(x)$ выпукла, то ее множества субуровня выпуклы для любого $\beta \in \mathbb{R}$. 
...
## Первый дифференциальный критерий выпуклости
...
## Второй дифференциальный критерий выпуклости

Двукратно дифференцируемая функция $f(x)$ определенная на выпуклом множестве $S \subseteq \mathbb{R}^n$ выпукла тогда и только тогда, когда $\forall x \in \mathbf{int}(S) \neq \emptyset$:
...
:::
::: {.column width="45%"}
![Сильно выпуклая функция больше или равна квадратичной аппроксимации Тейлора в любой точке](global_quad_lower_bound.pdf)
:::
::::
...
## Доказательство первого дифференциального критерия сильной выпуклости
...
## Доказательство первого дифференциального критерия сильной выпуклости
**Достаточность**: Предположим, что неравенство в теореме выполняется для всех $x, x_0 \in X$. Возьмем $x_0 = \lambda x_1 + (1 - \lambda) x_2$, где $x_1, x_2 \in X$, $0 \leq \lambda \leq 1$. Согласно неравенству, следующие неравенства выполняются:
...
## Доказательство второго дифференциального критерия сильной выпуклости

Целевое неравенство тривиально, когда $y = \mathbf{0}_n$, поэтому мы предполагаем $y \neq \mathbf{0}_n$.

**Необходимость**: Предположим, что $x$ является внутренней точкой множества $X$. Тогда $x + \alpha y \in X$ для всех $y \in \mathbb{R}^n$ и достаточно малых $\alpha$. Поскольку $f(x)$ дважды дифференцируема,
$$
...
**Достаточность**: Используя формулу Тейлора с остаточным членом Лагранжа и целевое неравенство, мы получаем для $x + y \in X$:
$$
...
Функция $f: \mathbb{R}^n \rightarrow \mathbb{R}$ называется **правильной выпуклой функцией**, если она никогда не принимает значения $-\infty$ и не равна $\infty$ тождественно.
...
является правильной выпуклой функцией.
:::
...
Функция $f: \mathbb{R}^n \rightarrow \mathbb{R}$ называется **замкнутой**, если для каждого $\alpha \in \mathbb{R}$, множество субуровня замкнуто.

Эквивалентно, если эпиграф замкнут, то функция $f$ замкнута.

:::

![Эта концепция позволяет корректно описывать поведение функций, которые могут иметь разрывы на границе своей области определения.](closed_function.pdf){width=90%}
...
* Неравенство Йенсена для выпуклых функций:
 ...
 Для случая бесконечной размерности:
...
$$
Интересно, что алгоритм градиентного спуска имеет линейную сходимость.

Следующие функции удовлетворяют условию PL, но не являются выпуклыми. [\faPython Link to the code](https://colab.research.google.com/github/MerkulovDaniil/optim/blob/master/assets/Notebooks/PL_function.ipynb)
...
## Метод наименьших квадратов или линейная регрессия

![Иллюстрация](lls_idea.pdf){width=75%}

В задаче метода наименьших квадратов или линейной регрессии у нас есть измерения $X \in \mathbb{R}^{m \times n}$ и $y \in \mathbb{R}^{m}$ и мы ищем вектор $\theta \in \mathbb{R}^{n}$ такой, что $X \theta$ близок к $y$. Близость определяется как сумма квадратов разностей: 
$$ 
...
## Метод наименьших квадратов или линейная регрессия ^[Посмотрите на [\faPython пример](https://colab.research.google.com/github/MerkulovDaniil/optim/blob/master/assets/Notebooks/Real_world_LLS_exercise.ipynb) реальных данных линейной задачи метода наименьших квадратов]

1. Является ли эта задача выпуклой? Сильно ли выпукла?
1. Что вы думаете о сходимости градиентного спуска для этой задачи?

## $l_2$-регуляризованный метод наименьших квадратов

В случае недоопределенной задачи часто желательно восстановить сильную выпуклость целевой функции, добавив $l_2$-штраф, также известный как регуляризация Тихонова, $l_2$-регуляризация или демпфирование весов.
...
![Выпуклая задача не имеет сходимости в области](lls_convex.pdf){width=85%}
...
![Но если добавить даже небольшое количество регуляризации, вы гарантируете сходимость в области](lls_convex_l2.pdf){width=85%}
...
![Другой способ обеспечить сходимость в предыдущей задаче - переключить значения размерности](lls_strongly_convex.pdf){width=85%}

## Для сходимости к решению с высокой точностью необходима сильная выпуклость (или выполнение условия Поляка-Лоясевича).

![Только маленькая точность может быть достигнута с сублинейной сходимостью](logreg_convex.pdf){width=85%}

## Для обеспечения сходимости с высокой точностью необходимо иметь сильную выпуклость (или PL)


![Сильная выпуклость обеспечивает линейное сходимость](logreg_strongly_convex.pdf){width=85%}
