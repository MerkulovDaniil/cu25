---
title: "Субградиентный метод. Негладкие задачи"
...
## Сходимость: выпуклый случай

::: {.callout-theorem}

Пусть $f$ — выпуклая функция, $x^*$ — точка минимума. Предположим, что $\|g_k\| \le G$ для всех $k$. Тогда:
$$
...
## Сходимость: выпуклый случай, постоянный шаг (1)
...
## Сходимость: выпуклый случай, постоянный шаг (2)
...
* Любопытно, что, если вы будете искать оптимальный шаг на каждой итерации: $\alpha_0, ..., \alpha_{k-1}$, то получите ту же оценку.
* Это связано с симметрией и выпуклостью правой части относительно $\alpha_i$.

## Сходимость: выпуклый случай, постоянный шаг (3)
...
1. Оценка сумм:
   $$
   \begin{gathered}
       \uncover<+->{\sum_{i=0}^{k-1}\alpha_i^2 = \frac{R^2}{G^2}\sum_{i=1}^{k}\frac{1}{i} \le \frac{R^2}{G^2}\bigl(1+\ln k\bigr);}
       \uncover<+->{\qquad \sum_{i=0}^{k-1}\alpha_i = \frac{R}{G}\sum_{i=1}^{k}\frac{1}{\sqrt{i}} \ge \frac{R}{G}\int_{1}^{k+1}\frac{1}{\sqrt{t}}\,dt = \frac{2R}{G}\bigl(\sqrt{k+1}-1\bigr). \\ }
...
## Разные комбинации (сильной) выпуклости и (не)гладкости
...
## Сильно выпуклая негладкая функция
...
Пусть $f: \mathbb{R}^n \to \mathbb{R}$ — выпуклая функция, и мы используем субградиентный метод. Пусть $\|g_k\| \le G$ для всех $k$, и $\|x_0 - x^*\| \le R$. Тогда для лучшего найденного значения функции $f_T^{\text{best}} = \min_{0 \le k < T} f(x_k)$ справедливо:
$$
f_T^{\text{best}} - f(x^*) \le \frac{R^2 + G^2 \sum_{k=0}^{T-1} \alpha_k^2}{2 \sum_{k=0}^{T-1} \alpha_k}.
$$
:::

**Доказательство**
Воспользуемся определением шага субградиентного метода $x_{k + 1} = x_k - \alpha_k g_k$ и неравенством, определяющим субградиент: $f(x^*) \ge f(x_k) + \left\langle g_k, x^* - x_k \right\rangle$:
$$
\begin{split}
...
## Сходимость: выпуклый случай (доказательство 2)

Просуммируем неравенство по $k$ от $0$ до $T-1$ и воспользуемся предположением на ограниченность нормы субградиента: $\left\| g_k \right\| \le G$:
$$
\begin{split}
...
$$
\uncover<+->{ $$
(f_T^{\text{best}} - f(x^*))\sum_{k=0}^{T-1}2\alpha_k \le \sum_{k=0}^{T-1}2\alpha_k (f(x_k) - f(x^*)) \le R^2 + G^2\sum_{k=0}^{T-1}\alpha_k^2
$$}
\uncover<+->{ Это дает оценку 
$$
f_T^{\text{best}} - f(x^*) \leq \frac{R^2 + G^2\sum \alpha_k^2}{2\sum \alpha_k}.
$$ 
Отсюда следует сходимость при $\sum \alpha_k^2 < \infty, \sum \alpha_k = \infty$ (шаг убывает, но не слишком быстро).
}
...
1. Для любого $\lambda\in[0,1)$, из $\mu$-сильной выпуклости,
   ...
   $$
2. Из неравенства субградиента в $x$, мы получаем
   $$
...
3. Таким образом,
   ...
   \end{aligned}
   $$
4. Переходя к пределу при $\lambda\to 1^-$ получаем $f(x) \le f(y) + \langle g,x-y\rangle - \frac{\mu}{2}\|x-y\|^2 \to \langle g,x-y\rangle \ge f(x)-f(y)+\frac{\mu}{2}\|x-y\|^2$.
