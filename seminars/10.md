...
1. Мы не можем получить явные выражения для $x_{k + 1}$ (без argmin) из \eqref{eq:cubreg} как в градиентном спуске.
...
## Сходимость \footnote{\href{https://arxiv.org/pdf/2410.04083}{Kamzolov, D., et al. (2024). Optami: Global superlinear convergence of high-order methods. ICLR 2025.}}
...
<!-- The secant equation $B_{k+1} d_k = g_k$ yields:
$$
(a u^T d_k) u = g_k - B_k d_k
$$ -->
...
<!-- This only holds if $u$ is collinear to $g_k - B_k d_k$. Putting $u = g_k - B_k d_k$, we solve the above, 
$$
a = \frac{1}{(g_k - B_k d_k)^T d_k},
$$ 
...
- $|(g_k - B_k s_k)^T s_k | \ge r \left\|s_k\right\| \left\|g_k - B_k s_k\right\|,\ 0 < r \ll 1$.
...
Multiplying by $g_k$, using the secant equation $d_k = C_k g_k$, and solving for $a$, $b$, yields:

$$
C_{k+1} = C_k - \frac{C_k g_k g_k^T C_k}{g_k^T C_k g_k} + \frac{d_k d_k^T}{g_k^T d_k}
$$
...
$$
B_{k+1} = \left(I - \frac{g_k d_k^T}{g_k^T d_k}\right)B_k\left(I - \frac{d_k g_k^T}{g_k^T d_k}\right) + \frac{g_k g_k^T}{g_k^T d_k}
$$
...
The secant equation $g_k = B_{k+1} d_k$ yields:
$$
g_k - B_k d_k = (a u^T d_k) u + (b v^T d_k) v
$$

. . .

Putting $u = g_k$, $v = B_k d_k$, and solving for a, b we get: -->
...
$$
\Delta z_k = s_k - B_k g_k
$$
We get it:
$$
\mu_k q_k q_k^\top g_k = \Delta z_k
$$
$$
\left(\mu_k \cdot q_k^\top g_k\right) q_k = \Delta z_k
$$
A possible solution is: $q_k = \Delta z_k$, $\mu_k = \left(q_k^\top g_k\right)^{-1}$.
Then an iterative amendment to Hessian’s evaluation at each iteration:
$$
\Delta B_k = \dfrac{(s_k - B_k g_k)(s_k - B_k g_k)^\top}{\langle s_k - B_k g_k , g_k\rangle}.
$$

## Davidon–Fletcher–Powell method

$$
\Delta B_k = \mu_1 s_k (s_k)^\top + \mu_2 B_k g_k (B_k g_k)^\top.
$$
$$
\Delta B_k = \dfrac{(s_k)(s_k )^\top}{\langle s_k , g_k\rangle} - \dfrac{(B_k g_k)( B_k g_k)^\top}{\langle B_k g_k , g_k\rangle}.
$$
...
$$
$$
\Delta B_k = \dfrac{(s_k)(s_k )^\top}{\langle s_k , g_k\rangle} - \dfrac{(B_k g_k)( B_k g_k)^\top}{\langle B_k g_k , g_k\rangle} + p_k p_k^\top.
$$ -->
